...
3. **Where an External is selectable:** search (indexed by id, name and kind, shown with a kind chip), the Needs column chips, the Neighborhood pane, and the ranked table. Selecting one sets the Center like an Application does and writes `external=<id>` to the URL hash.
