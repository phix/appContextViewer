...
const LAYOUTS = {
  ...
  cose:         { name: 'cose', animate: false, randomize: true, numIter: 1000 },
  ...
  fcose:        { name: 'fcose', animate: false, randomize: true, quality: 'default', nodeRepulsion: 4500, idealEdgeLength: 80 },
...
const results = [];
for (const lk of only) for (const n of sizes) for (const compound of (process.argv[4] === "cmp" ? [true] : process.argv[4] === "flat" ? [false] : [false, true])) {
...
import("fs").then(fs => fs.writeFileSync(`results/${only.join("_")}_${process.argv[4]||"both"}_${sizes.join("-")}.json`, JSON.stringify(results, null, 1)));
