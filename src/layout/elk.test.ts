...
describe('createOverviewLayout, elk directly in Node', () => {
  it('lays out the collapsed Overview: 123 Repositories with their Group Dependencies', async () => {
  ...
  it('lays out Expand all: 1,000 Applications inside 123 Repository boxes, every member inside its box', async () => {
  ...
  it('keeps nested Groups inside their parents and accepts an edge to a Group', async () => {
  ...
  it('reuses one worker across runs and rejects runs after dispose', async () => {
    const layout = createOverviewLayout();
  ...
  it('rejects an invalid spec with LayoutSpecError', async () => {
    const layout = createOverviewLayout();
    try {
      await expect(
